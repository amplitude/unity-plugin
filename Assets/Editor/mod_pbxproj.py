...
class PBXGroup(PBXType):
    ...
    @classmethod
    def Create(cls, name, path=None, tree=None):
        grp = cls()
        ...
        if path:
            grp['path'] = path
            grp['sourceTree'] = tree or 'SOURCE_ROOT'
        else:
            grp['sourceTree'] = tree or '<group>'
...
class XcodeProject(PBXDict):
    ...
    def get_or_create_group(self, name, path=None, parent=None, tree=None):
        if not name:
            return None
